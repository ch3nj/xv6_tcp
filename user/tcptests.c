...
//
static void
ping(uint16 sport, uint16 dport, int attempts, int isMac)
{
  int fd;
...
dns()
{
  ...
  printf("d1\n");
  if((fd = connect(dst, 10000, 53, SOCK_TYPE_TCP_CLIENT)) < 0){
    fprintf(2, "ping: connect() failed\n");
    exit(1);
  }
...
main(int argc, char *argv[])
{
  ...
  // if (argc > 1) {
  //   ping(2000, dport, 100, (int)*argv[1]);
  // } else {
  //   ping(2000, dport, 100, 0);
  // }
